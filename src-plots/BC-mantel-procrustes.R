...
# Load the data
data <- read.delim("BC-mantel-procrustes.tsv", header = TRUE, stringsAsFactors = TRUE, sep = "\t")
...
p2 <- ggplot(data, aes(x = Procrustes_M2, y = project_id, color = Disease_Level_1)) +
  ...
  theme(
    ...
    #axis.title.x = element_text(size = rel(0.8)),  # Reduce x-axis label size
    axis.title.x = element_blank(),  # Hide x-axis title
    panel.border = element_rect(color = "black", fill = NA),  # Add black border around each panel
    panel.spacing = unit(0.01, "cm"), # Minimize panel spacing
...
library(gridExtra)
BC_mantel_procrutes_plot <- grid.arrange(p1, p2, ncol = 2)
print(BC_mantel_procrutes_plot)

# Save plot
ggsave("BC_mantel_procrutes_with_bg.png", BC_mantel_procrutes_plot, dpi = 600, width = 8, height = 8)
